...
test("kicad-to-circuit-json: Edge.Cuts gr_curve fragment PCB", async () => {
  const kicadPcbPath = "tests/assets/gr-curve-edge-cuts-fragment.kicad_pcb"
  const kicadPcbContent = readFileSync(kicadPcbPath, "utf-8")

  const converter = new KicadToCircuitJsonConverter()
  converter.addFile("gr-curve-edge-cuts-fragment.kicad_pcb", kicadPcbContent)
  ...
  await fs.writeFile(
    "tests/pcb/__snapshots__/gr-curve-edge-cuts-fragment-circuit-json.json",
  ...
  await fs.writeFile(
    "tests/pcb/__snapshots__/gr-curve-edge-cuts-fragment-circuit-json.svg",
    circuitJsonSvg,
  )
  ...
  await expect(stackedPng).toMatchPngSnapshot(
    import.meta.path,
    "gr-curve-edge-cuts-fragment-pcb",
  )
}, 20_000)
