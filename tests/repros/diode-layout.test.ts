...
test("kicad-to-circuit-json: diode-layout PCB", async () => {
  // Load the KiCad PCB file
  const kicadPcbPath = "tests/assets/diode-layout.kicad_pcb"
  const kicadPcbContent = readFileSync(kicadPcbPath, "utf-8")

  // Convert to Circuit JSON
  const converter = new KicadToCircuitJsonConverter()
  converter.addFile("diode-layout.kicad_pcb", kicadPcbContent)
  ...
  await fs.writeFile(
    "tests/repros/__snapshots__/diode-layout-circuit-json.json",
  ...
  await fs.writeFile(
    "tests/repros/__snapshots__/diode-layout-circuit-json.svg",
    circuitJsonSvg,
  )
