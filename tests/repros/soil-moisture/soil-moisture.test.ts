...
test("kicad-to-circuit-json repro: Arduino Nano PCB", async () => {
  const kicadPcbPath = "tests/repros/soil-moisture/soil-moisture.kicad_pcb"
  const kicadPcbContent = readFileSync(kicadPcbPath, "utf-8")

  const converter = new KicadToCircuitJsonConverter()
  converter.addFile("soil-moisture.kicad_pcb", kicadPcbContent)
  ...
  await fs.mkdir("tests/repros/soil-moisture/__snapshots__", {
    recursive: true,
  })
  await fs.writeFile(
    "tests/repros/soil-moisture/__snapshots__/soil-moisture-circuit-json.json",
  ...
  await fs.writeFile(
    "tests/repros/soil-moisture/__snapshots__/soil-moisture-circuit-json.svg",
    circuitJsonSvg,
  )
